//! 传统路由匹配引擎 — 与 Kong traditional router 完全一致
//!
//! 匹配规则优先级（位值）：
//! HOST(0x40) > HEADER(0x20) > URI(0x10) > METHOD(0x08) > SNI(0x04)
//!
//! 路由排序：
//! submatch_weight > header数量 > regex_priority > URI长度 > created_at
...
pub struct TraditionalRouter {
    ...
    /// 按 category 分组的路由（已排序）
    categories: Vec<Category>,
    /// 精确 host -> 路由索引（用于快速索引优化）
    #[allow(dead_code)]
...
impl TraditionalRouter {
    ...
    pub fn new(routes: &[Route]) -> Self {
        ...
        // 排序 categories：match_weight 高的优先
    ...
    /// HEADER 匹配（所有 header 都必须匹配 = AND 逻辑）
    fn match_headers(
        &self,
        route: &ProcessedRoute,
...
fn sort_routes(a: &ProcessedRoute, b: &ProcessedRoute) -> std::cmp::Ordering {
    ...
    // 3. regex_priority（仅对有正则 URI 的路由）
    ...
    // 4. max_uri_length（更长的路径优先）
    let cmp = b.max_uri_length.cmp(&a.max_uri_length);
    if cmp != std::cmp::Ordering::Equal {
        return cmp;
    }

    // 5. created_at（FIFO，更早创建的优先）
...
mod tests {
    ...
    fn test_more_specific_route_wins() {
        ...
        r2.created_at = r1.created_at + 1; // 更晚创建
