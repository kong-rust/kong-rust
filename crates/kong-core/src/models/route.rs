...
/// Route entity — fully consistent with Kong routes table — Route 实体 — 与 Kong routes 表完全一致
...
pub struct Route {
    ...
    /// HTTPS redirect status code, default 426, options: 301/302/307/308 — HTTPS 重定向状态码，默认 426，可选 301/302/307/308
    pub https_redirect_status_code: u16,
    /// Whether to strip matched path prefix, default true — 是否去除匹配的路径前缀，默认 true
    pub strip_path: bool,
    /// Whether to preserve the original Host header, default false — 是否保留原始 Host 头，默认 false
    pub preserve_host: bool,
    /// Whether to enable request buffering, default true — 是否启用请求缓冲，默认 true
