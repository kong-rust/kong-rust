...
| **LLM 网关** | Lua 插件 | 完整（100+ provider） | **Rust 原生（4 插件、5 provider、智能路由）** |
...
- **多 Provider 支持** — OpenAI、Anthropic（Claude）、Gemini、通义千问、混元，以及任意 OpenAI 兼容服务
- **智能模型路由** — 正则匹配 + 加权轮询，支持 A/B 测试、成本优化、多区域分发
- **双协议暴露** — 同时暴露 OpenAI（`/v1/chat/completions`）和 Anthropic（`/v1/messages`）两种客户端协议
- **Token 限流** — 按 consumer/route/global 的 TPM/RPM 限制，预扣 + 修正机制保证计量准确
- **多模型负载均衡与 Fallback** — 加权轮询 + 优先级 fallback + 健康冷却机制
- **虚拟 API Key 管理** — 发行虚拟 key，设置预算/限额/模型白名单
- **Token 成本追踪** — 按模型定价，三级 token 计数（provider > tiktoken > 估算）
- **语义缓存** — SHA256 缓存键提取 + skip header（Redis 后端规划中）
- **Prompt Guard** — 正则 deny/allow 模式 + 消息长度限制，支持阻断或仅记录
- **流式 SSE** — 完整流式支持，跨 chunk 重组、TTFT 追踪、usage 累积
...
kong-server（主入口二进制）
 ...
 ├── kong-ai            — LLM 网关引擎：4 个插件（proxy、rate-limit、cache、prompt-guard）、5 个 provider、智能路由
...
| Phase 2a-MVP | B | **已完成** | LLM 网关 MVP — OpenAI 协议代理、Token 计数 |
| Phase 1 | A | 规划中 | Hybrid CP/DP 模式（传统网关封顶） |
| Phase 2a-Full | B | **已完成** | 多模型负载均衡与 Fallback（Anthropic、Gemini、通义千问、混元）+ 智能路由 |
| Phase 2b | B | **已完成** | 虚拟 API Key、Token 成本追踪 |
| Phase 2c | B | **已完成** | 语义缓存（缓存键基础设施） |
| Phase 2d | B | **已完成** | Prompt Guard（正则 deny/allow + 长度限制） |
