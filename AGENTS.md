...
6. 所有数据面功能的方案设计必须包含容量模型：目标 QPS/并发、租户或 key
   基数、数据增长与保留期、热路径 I/O、水平扩展状态归属、背压/降级、
   可观测性，以及负载与故障验证口径。
7. 高频共享状态和持续增长的请求事实必须通过可替换的 trait/adapter 隔离具体
   后端；不得把 `DashMap`、SQL、PostgreSQL sequence 或单机时钟等实现细节泄漏
   到领域与插件契约。内存/PG 可以作为默认后端，但不能成为企业部署的唯一演进
   路径。
8. 按数据语义选择存储：配置和强一致账本使用事务型存储；跨节点实时计数预留
   Redis 等原子分布式后端；海量 append-only 日志/事实预留外部日志或分析存储。
   不得用最终一致的日志后端承担预算等强一致决策。
